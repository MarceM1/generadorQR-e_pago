...
class epagos_api {
  ...
  /**
 * Genera una orden QR para el pago
 * @param array $datos Vector con los datos del pago
 * @return array
 * @throws EPagos_Exception
 */
public function generar_orden_qr($datos){
  if (empty($datos)){
      throw new EPagos_Exception('Debe indicar los datos del pago para generar la orden QR');
  }
  if (!$this->_cliente){
      throw new EPagos_Exception('Debe invocar primero al obtener_token');
  }
  ...
  $resultado = $this->_cliente->generar_orden_qr($this->get_version(), 'op_pago', $credenciales, $datos);

  if (self::DEBUG_ACTIVADO) {
      $this->_debug[] = 'generar_orden_qr :: ' . $this->_cliente->__getLastResponse();
  }
